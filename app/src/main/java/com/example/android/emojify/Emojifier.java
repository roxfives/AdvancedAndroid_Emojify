...
class Emojifier {
    ...
    private static void whichEmoji(Face face){
        ...
        boolean isSmiling = (face.getIsSmilingProbability() > SMILING_PROB_THRESHOLD)? true : false;
        boolean isLeftEyeOpened = (face.getIsLeftEyeOpenProbability() > EYE_OPEN_PROB_THRESHOLD)? true : false;
        boolean isRightEyeOpened = (face.getIsRightEyeOpenProbability() > EYE_OPEN_PROB_THRESHOLD)? true : false;

        Emoji emoji;
        if(isSmiling) {
            if (isLeftEyeOpened && !isRightEyeOpened) {
                emoji = Emoji.LEFT_WINK;
            }  else if(isRightEyeOpened && !isLeftEyeOpened){
                emoji = Emoji.RIGHT_WINK;
            } else if (isLeftEyeOpened){
        ...
        Log.d(LOG_TAG, "The emoji is " + emoji.toString());
    }
}
